...
const soul = [
  ...
  const title = widget.addText("舔狗日记")
  title.font = Font.ultraLightRoundedSystemFont(20)
  title.textColor = new Color('#f7e01e', 1)
  title.centerAlignText()
  widget.addSpacer(5)
  // 天气类型多种多样，一些极端天气如暴雪、暴雨、冰雹等需要在特定节气发生，为了效率与真实性考虑这里选用常见天气类型
  // 且增大晴、多云与阴雨天气概率（傻瓜式增大该条目数量），也可以传递 widget 参数来自定义天气
  const normalWeathers = [
    '晴', '多云', '阴', '小雨', '晴转阴', '大雨', '多云转晴', '晴', '阵雨', '晴转多云', '雷阵雨', '中雨', '晴', '小雨转阴', '小雨转中雨', '多云转晴', '多云转阴', '阴转小雨', '阴转晴', '晴', '阴', '多云', '阴转阵雨', '雷阵雨', '晴', '阵雨转多云', '晴', '阴', '多云', '阴转小雨', '多云转小雨', '小雨转中雨', '小雨转晴', '小雨转阵雨'
  ]
  let randomWeather = normalWeathers[Math.floor(Math.random() * normalWeathers.length)]
  const param = args.widgetParameter
  if (param != null && normalWeathers.includes(param)) {
    randomWeather = param
  }
  const df = new DateFormatter()
  df.dateFormat = 'M月d日'
  const dateStr = df.string(new Date())
  
  const note = widget.addText(dateStr + '  ' + randomWeather)
  note.font = Font.ultraLightSystemFont(12)
  note.textColor = new Color('#ffffff', 0.6)
  note.centerAlignText()
  
  widget.addSpacer(10)
  
  let diary = soul[Math.floor(Math.random() * soul.length)];
  diary = diary.replaceAll('**', '')
  const content = widget.addText('        ' + diary)
  content.font = Font.lightSystemFont(10)
  content.textColor = new Color('#ffffff', 1)
